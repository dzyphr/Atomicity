...
print("current gas price * 2:", rpc.eth.gas_price * 2);
...
if verifyBlockExplorer == True: #https://docs.etherscan.io/tutorials/verifying-contracts-programmatically
    ...
    url = os.getenv('GoerliScan') #block explorer api url
    ...
    if constructorArgs == False:
        ...
        content = {
                ...
                'optimizationUsed': 0, #0 or 1
                'runs': 200,
                'constructorArguements': '', #call the w3javascript with contract address (and name add this) 
                                            #and get abi encoded constructor
                'evmversion': '',
                'licenseType': 1
        }
...
#interacting via transaction

valueToStore = 256

storageTx = SimpleStorage.functions.store(valueToStore).buildTransaction(
    {
        "chainId": chain_id,
        "from": senderAddr,
        "nonce": rpc.eth.getTransactionCount(senderAddr),
...
signedStorageTx = rpc.eth.account.sign_transaction(
    storageTx, senderPrivKey
)

storageTxHash = rpc.eth.send_raw_transaction(signedStorageTx.rawTransaction)
print("storing the value:", valueToStore, "in the contract")

storageTxReceipt = rpc.eth.wait_for_transaction_receipt(storageTxHash)
print("value:", valueToStore, "stored!")

#interaction via call

storedVal = SimpleStorage.functions.retrieve().call()

print("stored value retrieved :", storedVal)
