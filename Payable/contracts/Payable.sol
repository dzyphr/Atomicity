...
contract Payable
{
	...
	function transfer(address payable reciever, uint amount) public //can be called by ANYONE!! UNSAFE!!
	{
		(bool success, ) = reciever.call{value: amount}("");
		require(success, "Failed to Send!");
	}

	function ownerOnlyTransfer(address payable reciever, uint amount) public //can only be called by contract owner! safer!
	{
		require(msg.sender == owner, "Caller is not sender/contract owner"); 
		(bool success, ) = reciever.call{value: amount}("");
                require(success, "Failed to Send!");
